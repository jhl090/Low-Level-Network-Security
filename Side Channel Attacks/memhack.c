...
int main(int argc, char **argv) {
    ...
    char *page_off = page_start;
    int i = 0;
    int j;
    while(i < 32) { // Validate password character by character 
      j = 0;
      page_off--;     
      memcpy(page_off, guess, i);
      ...
      for (j=j; j < 94; j++){
        ...
        if (sigsetjmp(jumpout,1) == 1) { 
          ...
          guess[i] = c;
          //printf("Guess: %s\n", guess);
          //puts("Calling checkpass . . . ");
          //printf("CHECKPASS RETURNS: %d\n", check_pass(guess));
          if (check_pass(guess)) { hack_system(guess); } //ONLY NEEDED ON IENG6
          i++; 
          break;
        }
        signal(SIGSEGV, SIG_DFL);
    ...
    //pri ntf("Could not get the password!  Last guess was %s\n", guess);
    return 1;
};
